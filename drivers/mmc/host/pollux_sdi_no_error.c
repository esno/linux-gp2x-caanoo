...
//static int m_channel = 0; // 0 channel
...
struct pollux_sdi_host {
	...
	int          cmdflags; // command command register
...
static int sdmmc_init(struct pollux_sdi_host *host)
{
    ...
    MES_SDHC_OpenModule(host->channel); // sdi control register interrupt enable bit. c000_9800, c000_c800

	// dmarst/fiforst, controller reset, sdi control register
    MES_SDHC_ResetController(host->channel);    // Reset the controller.



    // controller reset bit normal mode
    while (MES_SDHC_IsResetController(host->channel));  // Wait until the
                                                    // controller reset is
                                                    // completed.

	// fifo controller normal mode, DMA reset, contol register
    ...
    // command response timeout setting
...
static irqreturn_t pollux_sdi_irq(int irq, void *dev_id)
{
	...
	if( sdi_mint & SDI_INTSTS_CD )
	{
		...
		sdi_pclear |= SDI_INTSTS_CD; // pend clear data
	}
    ...
    if( host->mrq->data )
    {
    	if(host->mrq->data->flags & MMC_DATA_READ)
    	{
			...
			// Data Transfer Over Data transfer completed Data Timeout
			// ==>  32bytes SDI_INTSTS_RXDR
			if( sdi_mint & SDI_INTSTS_DTO )
            {
            	if( (host->length - host->bytes_process) >= 64 ) // 32bytes
            	{
...
static int sdi_set_command_flag(struct pollux_sdi_host *host)
{
    ...
    switch( command )
    {
    case GO_IDLE_STATE:        // No Response(mmc/sd)
        flag =
...
START_CMD:
	...
	p->RINTSTS = 0xFFFFFFFF; // all interrupt pend clear
	p->INTMASK = sdi_imsk;   // interrupt enable setting(write enable)
...
static int pollux_sdi_get_ro(struct mmc_host *mmc)
{
	...
	host->readonly =  pollux_gpio_getpin(host->wppin); // low write protected, high write enable ==> low active
...
static irqreturn_t pollux_sdi_irq_cd(int irq, void *dev_id)
{
	...
	new_cd_state = pollux_gpio_getpin(host->cdpin); // low/high
    ...
    else
    {
    	...
    	if( new_cd_state == 1 ) // high
    		set_irq_type( irq, IRQT_FALLING); // falling
    	else
...
static int pollux_sdi_probe(struct platform_device *pdev)
{
...
#if 0
	aprintk("pdev.id = %d\n", pdev->id);
	host->channel           = 0; // sdmmc channel 0
#else
	aprintk("pdev.id = %d\n", pdev->id);
	host->channel           = pdev->id; // sdmmc channel 0
#endif
	...
	MES_SDHC_Initialize(); //mes_sdhc.c block
...
//return 0; // ghcstop for debugging
	...
	host->readonly =  pollux_gpio_getpin(host->wppin); // low write protected, high write enable ==> low active
	host->cd_state = !pollux_gpio_getpin(host->cdpin); // low Inserted, high ejected. ==> 
...
#else
	aprintk("pdev.id = %d\n", pbdev->id);
	host->channel           = pbdev->id; // sdmmc channel 0
#endif
