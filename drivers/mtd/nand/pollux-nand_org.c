...
static int pollux_read_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf)
{
	...
	for(  i=0; i<4; i++ ) // 2k page
	{
       ...
       NF_CLEARECC();
		...
		pollux_nand_wait_dec(); // ecc

		if( pECC->reg_NFECCSTATUS & NAND_RDCHECKERR ) // error check, error
		{
			int dSyndrome[8];
			int ErrCnt, ErrPos[4];
...
static struct nand_ecclayout pollux_nand_oob_mlc_64 = {
	...
	static struct nand_ecclayout pollux_nand_oob_mlc_64 = {
		...
		.eccpos = {
			...
			32,33,34,35,36,37,38,39},
		.oobfree = {{0, 8}}   // file system reserved area
	};
