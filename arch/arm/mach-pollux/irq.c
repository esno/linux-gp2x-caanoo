...
{
	unsigned short mask = __raw_readw( POLLUX_VA_UART0 + POLLUX_UINTSTAT );
	mask |= ( 1 << (irqno - ( MAIN_IRQ_END + IRQ_UART0_OFFSET) ) );
	__raw_writew( mask, POLLUX_VA_UART0 + POLLUX_UINTSTAT);
...
{
	unsigned short mask = __raw_readw( POLLUX_VA_UART1 + POLLUX_UINTSTAT );
	mask |= ( 1 << (irqno - ( MAIN_IRQ_END + IRQ_UART1_OFFSET) ) );
	__raw_writew( mask, POLLUX_VA_UART1 + POLLUX_UINTSTAT);
...
{
	unsigned short mask = __raw_readw( POLLUX_VA_UART3 + POLLUX_UINTSTAT );
	mask |= ( 1 << (irqno - ( MAIN_IRQ_END + IRQ_UART3_OFFSET) ) );
...
pollux_irq_gpio_set_type(unsigned int irq, unsigned int type)
{
	...
	unsigned int   index    = gpio_nr >> 5;		   // 32
	unsigned int   residual = (gpio_nr%32);        // gpio A
	...
	// gpio mode, input, event type
	if( residual < 16 ) // low register setting
	{
		GPIO_FUNC_LOW(index)    &= ~( 3<<(residual*2) );       // 2bit clear == gpio mode
		GPIO_INOUT_MODE(index)  &= ~( 1<<residual );           // 1bit clear == gpio input mode

		GPIO_EVTMODE_LOW(index) &= ~( 3<<(residual*2) );       // 2bit clear == detection mode clear
	    GPIO_EVTMODE_LOW(index) |=  ( typeval<<(residual*2) ); // event detect mode setting
	...
	else
	{
		...
		GPIO_FUNC_HI(index)    &= ~( 3<<(residual*2) );       // 2bit clear == gpio mode
		GPIO_INOUT_MODE(index)  &= ~( 1<<residual );           // 1bit clear == gpio input mode

		GPIO_EVTMODE_HI(index) &= ~( 3<<(residual*2) );       // 2bit clear == detection mode clear
	    GPIO_EVTMODE_HI(index) |=  ( typeval<<(residual*2) ); // event detect mode setting
...
// gpioc[26] key_chg, sw3. low active
static void pollux_gpio_irq_demux( unsigned int irq, struct irq_desc *desc )
{
	...
	for(i=0; i<3; i++)
	{
		...
		tmp = gpio_event & gpio_mask; // mask
		gdebug("pend: 0x%08x, mask = 0x%08x\n", gpio_event, gpio_mask);
		#else
		tmp = GPIO_PEND(i) & GPIO_MASK(i); // mask
		#endif
...
{
	unsigned int   gpio_nr = IRQ_TO_GPIO(irq);	   // 0 ~ (32*3)-1
	unsigned int   mask = 1 << (gpio_nr & 0x1f);   // 0x1f mask
	unsigned int   index = gpio_nr >> 5;		   // 32
...
static void pollux_dma_irq_demux( unsigned int irq, struct irq_desc *desc )
{
	...
	for(i=0; i<16; i++)
	{
		...
		mask  = ((DMA_MODE(i)&POLLUX_DMA_INTMASK)>>1);  // Read dma enb  bit >> 1 => pend mask
		#else
		pend  = (DMA_MODE(i)&POLLUX_DMA_INTPEND);  // Read dma pend bit
		mask  = ((DMA_MODE(i)&POLLUX_DMA_INTMASK));  // Read dma enb  bit >> 1 => pend mask
...
// 070323: printk uart3 interrupt disable
// console (request_irq) interrupt
// serial source startup() interrupt enable startup serial open
void __init pollux_init_irq(void)
{
	...
	// printk clear
	//__raw_writew( 0x305, POLLUX_VA_UART3 + POLLUX_UCON); // 2410 patform device. level interrupt, debugging
	...
	__raw_writew( 0x0F, POLLUX_VA_UART0 + POLLUX_UINTSTAT); // 2410 patform device
	//__raw_writew( 0xFF, POLLUX_VA_UART0 + POLLUX_UINTSTAT); //  interrupt enable, debugging

	__raw_writew( 0x0F, POLLUX_VA_UART1 + POLLUX_UINTSTAT);

	//__raw_writew( 0xFF, POLLUX_VA_UART2 + POLLUX_UINTSTAT); //  interrupt enable, debugging
	__raw_writew( 0x0F, POLLUX_VA_UART2 + POLLUX_UINTSTAT);

	__raw_writew( 0x0F, POLLUX_VA_UART3 + POLLUX_UINTSTAT); //  interrupt disable, 070323 printk interrupt disable
	//__raw_writew( 0xFF, POLLUX_VA_UART3 + POLLUX_UINTSTAT); //  interrupt enable, debugging
